import { redactDocumentGeneratorPII } from '../../redact-pii/redact-pii-document-generator.js'
import { redactSFDMessagingProxyPII } from '../../redact-pii/redact-pii-sfd-messaging-proxy.js'
import { redactMessageGeneratorPII } from '../../redact-pii/redact-pii-message-generator.js'
import { redactApplicationStorageAccountTablesPII } from '../../redact-pii/redact-pii-application-storage-account-tables.js'
import { redactApplicationDatabasePII } from '../../redact-pii/redact-pii-application-database.js'
...
import { validateRedactPIISchema } from '../../messaging/schema/process-redact-pii-schema.js'

jest.mock('../../redact-pii/redact-pii-document-generator.js')
jest.mock('../../redact-pii/redact-pii-sfd-messaging-proxy.js')
jest.mock('../../redact-pii/redact-pii-message-generator.js')
jest.mock('../../redact-pii/redact-pii-application-storage-account-tables.js')
jest.mock('../../redact-pii/redact-pii-application-database.js')
jest.mock('../../redact-pii/update-application-redact-records.js')
jest.mock('../../redact-pii/create-redact-pii-flag.js')
jest.mock('../../redact-pii/get-applications-to-redact.js')
jest.mock('../../messaging/schema/process-redact-pii-schema.js')
...
describe('processRedactPiiRequest', () => {
  ...
  it('should call all redact functions and update records when no previous failed attempts', async () => {
    const apps = [{ id: 1 }]
    getApplicationsToRedact.mockResolvedValueOnce({
      applicationsToRedact: apps,
      status: ['applications-to-redact']
    })

    await processRedactPiiRequest(message, mockLogger)

    expect(redactDocumentGeneratorPII).toHaveBeenCalledWith(
      apps,
      ['applications-to-redact'],
      mockLogger
    )
    expect(redactSFDMessagingProxyPII).toHaveBeenCalledWith(
      apps,
      ['applications-to-redact', 'documents'],
      mockLogger
    )
    expect(redactMessageGeneratorPII).toHaveBeenCalledWith(
      apps,
      ['applications-to-redact', 'documents', 'messages'],
      mockLogger
    )
    expect(redactApplicationStorageAccountTablesPII).toHaveBeenCalledWith(
      apps,
      ['applications-to-redact', 'documents', 'messages', 'message-generator'],
      mockLogger
    )
    expect(redactApplicationDatabasePII).toHaveBeenCalledWith(
      apps,
      ['applications-to-redact', 'documents', 'messages', 'message-generator', 'storage-accounts'],
      mockLogger
    )
    expect(createRedactPIIFlag).toHaveBeenCalledWith(
      apps,
      [
        'applications-to-redact',
        'documents',
  ...
  it('should throw error when message validation fails', async () => {
    ...
    await expect(
      processRedactPiiRequest({ body: { requestedDate: 'invalid' } }, mockLogger)
    ).rejects.toThrow('Redact PII validation error')
  })
})
