...
import { createOrUpdateHerd } from '../ahwr/herd-processor.js'
...
const addClaimAndHerdToDatabase = async ({
  ...
  try {
    await session.withTransaction(async () => {
      ...
      //we don;t need full herd processing as not back-associating herds to existing claims
      const herdResult = await createOrUpdateHerd({
        herd,
        applicationReference,
      ...
      const status = await generateClaimStatus(dateOfVisit, logger, db) //might need to send scheme?
      ...
      // TODO: Decide on what to send out for mi report
...
export async function generateEventsAndComms(
  ...
  await publishStatusChangeEvent(logger, {
    ...
    claimType: type,
    typeOfLivestock,
    // reviewTestResults, //TODO: this outbound event needs a slightly different shape to support Poultry
    // piHuntRecommended,
    // piHuntAllAnimals,
    claimAmount: amount,
    dateTime: new Date(),
