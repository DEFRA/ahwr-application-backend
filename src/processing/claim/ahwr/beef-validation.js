...
const minimumAnimalsTestedForReview =
  minimumNumberOfAnimalsTested[TYPE_OF_LIVESTOCK.BEEF][claimType.review]
...
const optionalPiHuntModel = (
  isPositiveReviewTestResult,
  piHuntYes,
  piHuntRecommendedYes,
  piHuntRecommendedYesOrNotSet,
  piHuntAllAnimalsYes
) => {
  let validations = {}
  ...
  if (piHuntYes) {
    if (isPositiveReviewTestResult) {
      ...
      validations.piHuntRecommended = joi
        .string()
        .valid(piHuntRecommended.yes, piHuntRecommended.no)
        .required()
