...
const getMinNumberAnimalsTested = (minNumber) => ({
  numberAnimalsTested: joi.number().min(minNumber).required()
})
const getExactNumberAnimalsTested = (threshold) => ({
  numberAnimalsTested: joi.number().valid(threshold).required()
}) // this was not required previously. Should be?

const typeOfSamplesTaken = {
  typeOfSamplesTaken: joi
    .string()
    .valid(TYPES_OF_SAMPLE_TAKEN.oralFluid, TYPES_OF_SAMPLE_TAKEN.blood)
}
const numberOfOralFluidSamples = {
  numberOfOralFluidSamples: joi.number().when('typeOfSamplesTaken', {
...
const numberOfBloodSamples = {
  numberOfBloodSamples: joi.number().when('typeOfSamplesTaken', {
    is: TYPES_OF_SAMPLE_TAKEN.blood,
...
const vetVisitsReviewTestResults = {
  vetVisitsReviewTestResults: joi
    .string()
    .valid(testResultsConstant.positive, testResultsConstant.negative)
    .optional()
}
...
const pigsPcrTestResult = {
  pigsPcrTestResult: joi.string().when('pigsFollowUpTest', {
    is: 'pcr',
    then: joi.string().valid('positive', 'negative').required()
  })
}
const pigsElisaTestResult = {
  pigsElisaTestResult: joi.string().when('pigsFollowUpTest', {
    is: 'elisa',
    then: joi.string().valid('positive', 'negative').required()
  })
}
...
const biosecurityData = {
  biosecurity: joi
    ...
    .try(
      ...
      joi.object({
        biosecurity: joi.string().valid(biosecurity.yes),
        assessmentPercentage: joi.string().pattern(/^(?!0$)(100|\d{1,2})$/)
      })
    )
    .required()
}
