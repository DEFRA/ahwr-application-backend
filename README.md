...
An environment variable needs to be setup on the CDP portal (or local) with the name `DATA_CHANGES_DATA`. The variable will containe an JSON object with two keys, `version` and `data`. The value of `version` will be the number of the ticket. The `data` value will be an array of objects as per the definition found in src/distributed-jobs/data-changes/schema.js
